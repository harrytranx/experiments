...
ALIGNER_PARENT_DIR=/fsx_0/user/tranx/fbcode
ALIGNER_DEP_DIR=/fsx_0/user/tranx/fbcode/llm_mm_aligner/replicated
...
# MM9_CONF_DIR="/fsx_0/user/tranx/fbcode/llm_mm_aligner/experiments/aws/mm9_stage1"
MM9_CONF_DIR="/fsx_0/user/tranx/experiments/llm_mm_aligner/stage1_mm9"
# JSON_CONFIG=$MM9_CONF_DIR/pretrain_MM9_70B_Llama31_336px_128nodes_bz32_fix_norm.json
...
unset PYTHONPATH
export PYTHONPATH=${PYTHONPATH}:${ALIGNER_PARENT_DIR}:${ALIGNER_DEP_DIR}:${CONDA_PYTHON_PKGS}
echo $PYTHONPATH 

# PYTHONPATH=${PYTHONPATH}:${ALIGNER_PARENT_DIR}:${ALIGNER_DEP_DIR}:${CONDA_PYTHON_PKGS} srun --cpus-per-gpu 24 torchrun \
srun --cpus-per-gpu 24 torchrun \
    --nnodes ${SLURM_NNODES} \
    --nproc_per_node 8 \
    --rdzv_id $RANDOM \
    --rdzv_backend c10d \

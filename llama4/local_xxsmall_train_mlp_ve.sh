#!/bin/bash
export PYTHONPATH="$HOME/xlformers:$PYTHONPATH"
DUMP_DIR=/tmp/xlformers/tranx/xxsmall_mlp_ve/
mkdir -p $DUMP_DIR
rm -rf $DUMP_DIR/*
NCCL_CTRAN_ENABLE=0 XLFORMERS_ON_DEVGPU=1 torchrun --nproc-per-node 8 --standalone projects/pretrain/train.py \
--eager_init=False \
--dump_dir=$DUMP_DIR \
--enable_ods=True \
...
--model.modalities.image.recompute_transformer=False \
